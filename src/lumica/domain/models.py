...
class CloudFile(Base, TimestampMixin):
    ...
    status = Column(String, nullable=False, default="uploading", server_default=text("'uploading'"))
...
class CloudChunk(Base, TimestampMixin):
    ...
    status = Column(String, nullable=False, default="uploaded", server_default=text("'uploaded'"))
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
